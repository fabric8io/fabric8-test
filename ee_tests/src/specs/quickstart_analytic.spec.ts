...
//  });
  it('Create a new space, new Vert.x - HTTP & Config Map quickstart, run its pipeline', async () => {
    await runTest(landingPage, 'Vert.x - HTTP & Config Map', 'Components: Total: 2 | Analyzed: 2 | Unknown: 0').catch(error => console.log(error));
  });
...
//  });
 // tslint:enable:max-line-length
  ...
  async function runTest (theLandingPage: LandingPage, quickstartName: string, expectedReportSummary: string) {
    ...
    await browser.sleep(support.WAIT);
    dashboardPage.header.recentItemsDropdown.accountHomeItem.clickWhenReady();
    await browser.sleep(support.WAIT);
    dashboardPage.header.recentItemsDropdown.clickWhenReady();

    await browser.sleep(support.WAIT);
    dashboardPage.recentSpaceByName(spaceName).click();
    await browser.sleep(support.WAIT);
    spaceDashboardPage.stackReportsButton.click();
    await browser.sleep(support.WAIT);

    try {
      await expect(spaceDashboardPage.detailedReportHeading.getText()).toContain(expectedReportSummary);
    } catch (e) {
        support.writeScreenshot('target/analytic_report_fail_' + spaceName + '.png');
    }
