...
describe('openshift.io End-to-End POC test - Scenario - CREATE project - Run Che: ', function () {
  ...
  it("should perform Burr's demo - CREATE project - Create Che Worksdpace, Run Quickstart in Che", function() {
    ...
    OpenShiftIoChePage.clickBottomPanelTerminalTab();

    /* Execute the maven build command */
    testSupport.enterText (OpenShiftIoChePage.bottomPanelTerminal, "ls -l ");
    testSupport.enterText (OpenShiftIoChePage.bottomPanelTerminal, "cd " + spaceTime);
    testSupport.enterText (OpenShiftIoChePage.bottomPanelTerminal, "ls -l ");
    testSupport.enterText (OpenShiftIoChePage.bottomPanelTerminal, "mvn test ");

    /* TODO - Fiond a better way to wait for the mvn command to be successfully completed */
    browser.sleep(constants.LONG_WAIT);

    /* This fails with an error of: actualText.replace is not a function */
    //browser.wait(until.textToBePresentInElement(OpenShiftIoChePage.bottomPanelTerminalConsoleLines, 'BUILD SUCCESS'), constants.LONG_WAIT);
    
    OpenShiftIoChePage.bottomPanelTerminalConsoleLines.getText().then(function (text) {
      console.log("terminal console text = [[[" + text + "]]]");
    
      /* Verify that the build was successful */
      expect(text.toString()).toContain("BUILD SUCCESS");
    });
