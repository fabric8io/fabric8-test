...
# Exec EE tests
docker exec "$TEST_CONTAINER_NAME" ./ts-protractor.sh "$TEST_SUITE" | tee target/protractor.log
...
grep "TimeoutError" target/protractor.log
ret1=$?

# We do want to see that zero specs have failed
grep "0 failures" target/protractor.log
ret2=$?
