...
if test ! $(which pytest)
    ...
    then
        ...
        echo ""
        cd src/
        pytest test_gen_testdb.py -s -v --junitxml=$JENKINSLOG --sut=$1 --userid=$2 --offline_token=$3 2>&1 | tee $CLILOG
        echo ""
    elif [ "$4" = "True" ] || [ "$4" = "true" ]
    then
        ...
        echo ""
        cd src/
        pytest test_planner.py -s -v --junitxml=$JENKINSLOG --sut=$1 --userid=$2 --offline_token=$3 --cleanup=$4 2>&1 | tee $CLILOG
        echo $?
    fi
fi
